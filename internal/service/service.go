...
type Service struct {
	...
	State store.State
	// claimMu serializes task claims so the lease transition, capacity check
	// and persistence event commit as one critical section. This prevents two
	// concurrent claimants for the same device from both observing a queued
	// task as leaseable and committing independent leases (duplicate owners).
	claimMu sync.Mutex
}

func New(st store.Store, c ports.Clock, ids ports.IDGenerator) *Service {
	s := &Service{St: st, Clock: c, IDs: ids, State: store.EmptyState()}
	s.Recover()
	return s
}
...
func (s *Service) apply(e events.Event) {
	...
	case "task.updated":
		...
		if json.Unmarshal(e.Data, &t) == nil {
			s.State.Tasks[t.ID] = &t
			if t.Status == domain.TaskRollbackQueued {
				s.recordRollbackProgress(&t)
			}
		}
...
func (s *Service) recordRollbackProgress(t *domain.Task) {
	if t == nil || t.ReleaseID == "" || t.Status != domain.TaskRollbackQueued {
		return
	}
	if s.State.RollbackBatches == nil {
		s.State.RollbackBatches = map[string]store.RollbackBatch{}
	...
	queued := 0
	for _, current := range s.State.Tasks {
		if current.ReleaseID == t.ReleaseID && current.Status == domain.TaskRollbackQueued {
			queued++
		}
	}
	checkpoint := s.State.RollbackBatches[t.ReleaseID]
	if queued > checkpoint.Queued {
		checkpoint.Queued = queued
	}
	if checkpoint.Reason == "" {
		checkpoint.Reason = t.Error
	}
	s.State.RollbackBatches[t.ReleaseID] = checkpoint
}
